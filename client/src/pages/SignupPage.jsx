...
function SignupPage() {
  ...
  const { signup, oauthLogin, isAuthenticated } = useAuth()
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      setError(submitError.message || 'User already exists')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <AuthSplitLayout
      ...
      subtitle="Start analyzing and monitoring fairness in minutes."
    ...
    >
      <form className="space-y-4" onSubmit={onSubmit}>
        <label className="block text-sm">
          <span className="mb-2 block text-gray-700 dark:text-gray-200">Name</span>
          <input
            value={name}
        ...
        {isGoogleEnabled && (
          <>
            ...
            {isGoogleLoading && <p className="text-center text-xs text-gray-400">Signing up with Google...</p>}
          </>
        )}
      </form>
    </AuthSplitLayout>
